...
class Wang_100(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv1d(in_channels=1, out_channels=512, kernel_size=3, stride=1, padding=1)
...
if __name__ == '__main__':
    ...
    model=Transformer1D() #实例化网络模型
    model=model.to(device) #将模型转移到cuda上
    input=torch.ones((1,1,100)) #生成一个batchsize为64的，1个通道的10X10 tensor矩阵-可以用来检查网络输出大小
    input=input.to(device) #将数据转移到cuda上
    output=model(input) #将输入喂入网络中进行处理
    print(output.shape)
    summary(model,input_size=(1,100)) #输入一个通道为1的10X10数据，并展示出网络模型结构和参数
